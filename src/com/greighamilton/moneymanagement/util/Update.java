...
public class Update {
	...
	public static void doUpdate(Context c) {
		...
		// only update the locale if version is version 2 (do it once only)
		if (version < 3) setCurrency(c, sp);
		
		// only update the locale if version is version 3 (do it once only)
		if (version < 4) fixColorParserAgain(c, sp);
	}
	 ...
	 * @param sp			the app shared preferences
	 */
	private static void fixColorParserAgain(Context context, SharedPreferences sp) {
		...
		sp.edit().putInt("VERSION", 4).commit();
	}
...
private static void setCurrency(Context context, SharedPreferences sp) {
		...
		sp.edit().putInt("VERSION", 3).commit();
	}
